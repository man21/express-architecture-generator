...
//     // args['!'] = unknown
//     main(answers['project-name'], exit)
//   });
// parse args
var unknown = []
var args = parseArgs(process.argv.slice(2), {
...
function createApplication(name, dir, options, done) {
  ...
  //hstc
  app.locals.modules.hsts = 'hsts'
  app.locals.uses.push('hsts({ maxAge: 5184000 })')
  pkg.dependencies['hsts'] = '^2.2.0'

  //helmet
  app.locals.modules.helmet = 'helmet'
  app.locals.uses.push('helmet()')
  pkg.dependencies['helmet'] = '^4.6.0'

  //compression
  app.locals.modules.helmet = 'compression'
  ...
  // copy API templates
  // mkdir(dir, 'api')
  // copyTemplateMulti('nodejs/api', dir + '/api', '*.js')


  copyFolder(path.join(TEMPLATE_DIR, 'nodejs'), dir)

  // mkdir(dir, 'views')
  // copyFolder(path.join(TEMPLATE_DIR, 'nodejs/views'), path.join(dir, 'views'))

  // // copyTemplate('nodejs/views/index.html', path.join(dir, 'views/index.html'))

  // mkdir(dir, 'api')
  // copyFolder(path.join(TEMPLATE_DIR, 'nodejs/api'), path.join(dir, 'api'))


  // mkdir(dir, '.husky')

  // copyTemplate('nodejs/.husky', path.join(dir, 'views/index.html'))

  if (options.view) {
    // Copy view templates
    // mkdir(dir, 'views')
    // copyTemplateMulti('nodejs/views', dir + '/views', '*.html')
  ...
  copyTemplate('nodejs/server.js', path.join(dir, 'server.js'))
...
function main(options, done) {
  ...
  if (options['!'].length > 0) {
    ...
    // Path
    var destinationPath =  `${process.cwd()}/${options._[0] || "test"}` //options._[0] || '.'
